...
export default function NavAurora() {
  ...
  useEffect(() => {
    ...
    const beams: AuroraBeam[] = Array.from({ length: 4 }, (_, i) => ({
      x: 0.12 + i * 0.19,
      speed: 0.12 + Math.random() * 0.08,
      width: 0.14 + Math.random() * 0.1,
      hue: 115 + Math.random() * 18,
      alpha: 0.014 + Math.random() * 0.012,
    ...
    const draw = () => {
      ...
      for (const beam of beams) {
        const cx =
          w * (beam.x + Math.sin(time * beam.speed + beam.offset) * 0.1)
        const rw = w * beam.width
        const alpha =
          beam.alpha + Math.sin(time * 0.35 + beam.offset) * beam.alpha * 0.5
  ...
  return (
    <canvas
      ref={canvasRef}
      className="pointer-events-none absolute inset-0 rounded-[calc(1.75rem-1px)] mix-blend-screen opacity-80"
      aria-hidden="true"
    />
  )
}
