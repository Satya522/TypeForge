...
const COMMUNITY_RELAY_RETRY_MS = 4000;
...
const INLINE_COMMANDS: InlineCommand[] = [
  {
    command: "/race",
    label: "Start a typing race",
    description: "Spin up a live race lobby for this channel.",
  },
  {
    command: "/raid",
    label: "Summon a boss fight",
    description: "Call the squad into a co-op typing raid.",
  },
  {
    command: "/join",
    label: "Join a lobby code",
    description: "Use /join [code] to jump into a queued race.",
  },
  {
    command: "/wpm",
    label: "Post your WPM card",
    description: "Share a rich flex card with speed and accuracy.",
  },
  {
    command: "/challenge",
    label: "Challenge a member",
    description: "Use /challenge @user to call someone out.",
  },
];

const STATUS_OPTIONS: Array<{
  value: Exclude<Presence, "racing">;
  label: string;
}> = [
...
function CommunityClientPremium({
  ...
  const [presence, setPresence] =
    useState<Exclude<Presence, "racing">>("online");
  const [showCommandPalette, setShowCommandPalette] = useState(false);
  const [notifications, setNotifications] = useState<NotificationItem[]>([]);
  const [dismissedTickerRaceIds, setDismissedTickerRaceIds] = useState<
    string[]
  >([]);
  ...
  const [dismissedProfileNudge, setDismissedProfileNudge] = useState(
    Boolean(session?.user?.profileNudgeDismissed),
  );
  ...
  const currentUserName =
    profile?.name || getDisplayName(session?.user, "Typist");
  const currentAvatarUrl =
    profile?.avatar || getResolvedAvatarUrl(session?.user);
  ...
  const currentRank = normalizeRank(
    initialStats?.rank || profile?.rankTier,
  ...
  personalBestWpmRef.current = Math.max(
    initialStats?.bestWpm || 0,
    pb.wpm || 0,
  );
  ...
  const favoriteChannel =
    ...
    Object.entries(messagesByChannel)
      .map(([channelId, list]) => ({
        channelId,
        count: list.filter((message) => message.userName === currentUserName)
          .length,
      }))
      .sort((left, right) => right.count - left.count)[0]?.channelId ||
    "general";

  const derivedPresence: Presence = activeRace?.players.some(
  ...
  const channels = baseChannels.map((channel) => {
    const raceInChannel = liveRaces.find(
      (race) => race.channelId === channel.id && race.status !== "finished",
    );
    return {
      ...channel,
  ...
  const activePinnedMessages = activeMessages.filter(
  ...
  const globalTickerRace = liveRaces.find(
    (race) =>
      race.status !== "finished" && !dismissedTickerRaceIds.includes(race.id),
  );
  ...
  const wpmCardsThisWeek = Object.values(messagesByChannel)
    ...
    .map((message) => ({
      message,
      special: parseSpecialMessage(message.content),
    }))
    .filter(isWpmSpecial)
    .filter(
      (entry) =>
        new Date(entry.message.timestamp).getTime() >=
        Date.now() - 7 * 24 * 60 * 60 * 1000,
    );

  const activityCounts = todayMessages.reduce<Record<string, number>>(
    (accumulator, message) => {
  ...
  const dailyPulseMostActive = Object.entries(activityCounts).sort(
  ...
  const fastestToday =
    todayMessages
      ...
      .filter(isWpmSpecial)
      .sort(
        (left, right) => right.special.payload.wpm - left.special.payload.wpm,
      )[0] || null;
  ...
  const mostImprovedMember = (() => {
    ...
    return (
      Array.from(progressMap.entries())
        ...
        .sort((left, right) => right.gain - left.gain)[0]?.name ||
      currentUserName
    );
  })();

  const sortedMembers = [...onlineUsers].sort(
  ...
  function handleIncomingMessage(message: CommunityMessage) {
    setMessagesByChannel((previous) => {
      const existed = (previous[message.channelId] || []).some(
        (entry) => entry.id === message.id,
      );
      const next = {
        ...previous,
        [message.channelId]: withMessage(
          previous[message.channelId] || [],
          message,
        ),
      };
  ...
  function handleCommandSubmit(input: string) {
    ...
    if (trimmed.startsWith("/join")) {
      ...
      const match = liveRaces.find(
        (race) =>
          race.id === code ||
          race.id.slice(0, 6).toLowerCase() === (code || "").toLowerCase(),
      );
  ...
  function getMemberActivity(member: CommunityProfile) {
    const inRaid = liveRaids.some(
      (raid) =>
        raid.status !== "defeated" &&
        raid.players.some((player) => player.name === member.name),
    );
    if (inRaid) return "in boss fight";

    const inRace = liveRaces.some(
      (race) =>
        race.status !== "finished" &&
        race.players.some((player) => player.name === member.name),
    );
  ...
  function getMemberPresence(member: CommunityProfile): Presence {
    ...
    if (member.status === "idle" || member.status === "invisible")
      return "away";
    return "online";
  }
  ...
  useEffect(() => {
    ...
    function scheduleReconnect() {
      ...
      reconnectTimeout = window.setTimeout(() => {
        reconnectTimeout = null;
        void connectWhenRelayIsReady(true);
      }, COMMUNITY_RELAY_RETRY_MS);
    }
    ...
    async function connectWhenRelayIsReady(forceHealthCheck = false) {
      if (cancelled) return;
      ...
      const relayAvailable = await isCommunityRelayAvailable(forceHealthCheck);
      if (cancelled) return;
    ...
    function handleProfile(incomingProfile: CommunityProfile) {
      ...
      if (
        incomingProfile.status === "idle" ||
        incomingProfile.status === "invisible"
      )
        setPresence("away");
    ...
    function handleHistory(payload: {
      ...
      setMessagesByChannel((previous) => ({
        ...previous,
        [payload.channelId]: payload.messages.sort(
          (left, right) => left.timestamp - right.timestamp,
        ),
      }));
    }
    ...
    function handleRaceSnapshot(race: RaceState) {
      if (
        ...
        race.players.some(
          (player) => player.name === currentUserNameRef.current,
        )
      ) {
    ...
    function handleRaceProgress(payload: {
      raceId: string;
      players: RacePlayer[];
    }) {
  ...
  useEffect(() => {
    const mappedStatus =
      presence === "online" ? "online" : presence === "focus" ? "dnd" : "idle";
  ...
  return (
    <>
      <div className="flex h-screen overflow-hidden bg-[#08090b] text-white">
          ...
          <div className="border-b border-white/5 p-4">
            <div className="flex items-start justify-between gap-3">
              <UserIdentityCard
                ...
                bestWpm={Math.round(
                  Math.max(initialStats?.bestWpm || 0, pb.wpm || 0, currentWpm),
                )}
            ...
            >
              ...
              <span className="text-[11px] font-normal text-[#22c55e]">
                Show off
              </span>
            </button>
        ...
        <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setReplyTarget(
                      activePinnedMessages[activePinnedMessages.length - 1],
                    );
                  }}
          ...
          {activeChannelRace ? (
            <div className="border-b border-white/5 bg-white/[0.025] px-6 py-2">
              <div className="flex items-center gap-2 text-[12px] text-slate-400">
                ...
                />
                <span>
                  {activeChannelRace.participantCount} members are racing in{" "}
                  {activeChannel.name} right now.
                </span>
              </div>
            </div>
          ) : null}
          ...
          <LiveRaceTicker
            ...
            raceChannelName={
              globalTickerRace?.channelName || activeChannel.name
            }
            ...
            onClose={() => {
              if (!globalTickerRace) return;
              setDismissedTickerRaceIds((previous) => [
                ...previous,
                globalTickerRace.id,
              ]);
            }}
                  ...
                  <DailyPulse
                    ...
                    fastestWpm={
                      fastestToday?.special.kind === "wpm"
                        ? fastestToday.special.payload.userName
                        : currentUserName
                    }
                    fastestWpmValue={
                      fastestToday?.special.kind === "wpm"
                        ...
                        : Math.round(
                            Math.max(currentWpm, initialStats?.bestWpm || 0),
                          )
                    }
                ...
                {activeMessages.map((message) => {
                    ...
                    return (
                      <div key={message.id} className="flex gap-3">
                        <ProfileHoverCard
                          ...
                          fallbackName={
                            message.userName || parsed.payload.userName
                          }
                        >
                          <Avatar
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <WPMFlexCard
                            ...
                            onChallenge={() =>
                              createChallengeDraft(parsed.payload.userName)
                            }
                            onGhostRace={
                              activeChannelId === "speed-runs"
                                ? () =>
                                    createGhostChallenge(
                  ...
                  if (parsed?.kind === "streak") {
                    return (
                      <StreakFlexBanner
                        ...
                        isCurrentUser={
                          parsed.payload.userName === currentUserName
                        }
                        onReact={() => {
                  ...
                  if (message.type === "system") {
                    return (
                      <p
                        key={message.id}
                        className="py-1 text-center text-[11px] text-slate-500/70"
                      >
                        — {message.content} —
                      </p>
                    );
                ...
                {activeTypingUsers.length > 0 ? (
                  <p className="text-center text-[12px] text-slate-500">
                    <span className="text-slate-300">
                      {activeTypingUsers.map((user) => user.name).join(", ")}
                    </span>{" "}
                    typing...
                  </p>
                ) : null}
                      ...
                      >
                        {INLINE_COMMANDS.filter((entry) =>
                          entry.command.includes(
                            draft.trim().toLowerCase().split(/\s+/)[0] || "/",
                          ),
                        ).map((entry) => (
                          <button
                            key={entry.command}
                    ...
                    {sortedMembers.length === 0 ? (
                      ...
                      <div className="space-y-0.5">
                        {sortedMembers.map((member) => {
                          ...
                          const memberRank = normalizeRank(
                            member.rankTier,
                            member.wpm,
                          );
                            ...
                            >
                              <div className="flex items-start gap-3">
                                <ProfileHoverCard
                                  ...
                                  onChallenge={() =>
                                    createChallengeDraft(member.name)
                                  }
                                ...
                                {isExpanded ? (
                                  ...
                                  >
                                    <div className="mt-2 rounded-[16px] bg-black/20 px-3 py-2.5 text-xs text-slate-400">
                                      <p>
                                        {Math.round(member.accuracy)}% accuracy
                                        snapshot
                                      </p>
                                      <p className="mt-1">
                                        Current channel:{" "}
                                        {formatChannelName(
                                          member.currentChannel ||
                                            activeChannelId,
                                        )}
                                      </p>
                                    </div>
      ...
      <LiveRaceOverlay
        ...
        isCreator={Boolean(
          activeRace?.creatorId &&
          activeRace.creatorId === socketRef.current?.id,
        )}
