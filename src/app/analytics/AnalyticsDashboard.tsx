...
const rangeOptions: Array<{ helper: string; label: string; value: AnalyticsRange }> = [
    ...
    >
      <Reveal>
        <section className="panel relative overflow-hidden border border-white/10 bg-[linear-gradient(135deg,rgba(5,10,9,0.98),rgba(8,12,16,0.94)_48%,rgba(12,10,18,0.94))] shadow-[0_40px_120px_rgba(0,0,0,0.48)]">
          ...
          <div className="pointer-events-none absolute inset-0 opacity-[0.06]" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,0.12) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.12) 1px, transparent 1px)", backgroundSize: "46px 46px" }} />
            ...
            <div className="grid gap-8 xl:grid-cols-[1.16fr_0.84fr] xl:items-start">
              <div>
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <ToneBadge label={model.header.statusPill} tone={telemetryMode === "live" ? "cyan" : "amber"} />
              ...
              <div className="rounded-[1.9rem] border border-white/10 bg-black/25 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                <div className="flex items-start justify-between gap-5">
                  <div>
                    <p className="text-[0.68rem] font-semibold uppercase tracking-[0.34em] text-gray-500">Signal readiness</p>
                    <p className="mt-3 text-6xl font-black tracking-tight text-white">{model.header.readiness}%</p>
                  </div>
                ...
                <div className="mt-5 h-3 rounded-full bg-white/[0.06]">
                  <div className="h-full rounded-full bg-gradient-to-r from-[#39ff14] via-cyan-300 to-white" style={{ width: `${model.header.readiness}%` }} />
                </div>
                  ...
                  {rangeOptions.map((option) => (
                    ...
                    >
                      <span className="flex flex-col gap-1">
                        <span className="text-sm font-semibold uppercase tracking-[0.22em]">{option.label}</span>
                        <span className="text-[0.7rem] font-medium tracking-[0.16em] text-gray-500">{option.helper}</span>
                      </span>
                    </Button>
                  ))}
                </div>
                {isPending ? <p className="mt-3 text-sm text-cyan-200">Recomputing the intelligence layer for this range.</p> : null}
              </div>
            </div>
          </div>
        </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.12fr_0.88fr]">
        ...
        <div className="space-y-6">
          ...
          <Reveal delay={0.08}>
            <SessionReplayCard events={model.sessionReplay.events} summary={model.sessionReplay.summary} />
          </Reveal>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <Reveal>
          <TypingDNAProfile profile={model.typingDNA} unlockCount={model.unlocks.dna} />
        </Reveal>
        <Reveal delay={0.04}>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Reveal>
          <RhythmTimelineCard timeline={model.rhythmTimeline} unlockCount={model.unlocks.replay} />
        </Reveal>
        <div className="space-y-6">
          <Reveal delay={0.04}>
            <PressureResponseCard model={model.pressure} unlockCount={model.unlocks.pressure} />
          </Reveal>
          <Reveal delay={0.08}>
            <FocusDriftCard
      ...
      <Reveal>
        <ErrorHeatmapCard data={heatmapData} telemetryMode={model.telemetryMode} weakZones={model.topWeakZones} />
      </Reveal>
      ...
      <Reveal>
        <div className="rounded-[1.7rem] border border-white/10 bg-[linear-gradient(135deg,rgba(11,15,18,0.96),rgba(8,10,13,0.92))] px-6 py-6 shadow-[0_25px_80px_rgba(0,0,0,0.34)]">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-[0.68rem] font-semibold uppercase tracking-[0.34em] text-gray-500">Launch note</p>
              <h3 className="mt-2 text-2xl font-black tracking-tight text-white">Analytics now behaves like a premium typing lab.</h3>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-gray-400">
                It is purpose-built around typing pace, control, recovery, rhythm, weak zones, focus drift, and streak quality. Even low-data states stay intentional and premium.
              </p>
            </div>
            <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-gray-400">
